...
public class PlayerLedgeClimbState : PlayerState
{
    ...
    private Vector2 DetermineCornerPosition()
    {
        ...
        RaycastHit2D yHit = Physics2D.Raycast(CollisionSenses.LedgeCheckHorisontal.position + (Vector3)(workspace), Vector2.down, CollisionSenses.LedgeCheckHorisontal.position.y - CollisionSenses.WallCheck.position.y + 0.015f, CollisionSenses.WhatIsGround);
        float yDist = yHit.distance;

        workspace.Set(CollisionSenses.WallCheck.position.x + (xDist * Movement.FacingDirection), CollisionSenses.LedgeCheckHorisontal.position.y - yDist);
        return workspace;
    }
}
